...
if($Help.IsPresent)
{
    ...
    Write-Host '--------------------------------------------------------------------------------'
    Write-Host;
    Write-Host 'This script will execute the *.sql load control execution files in the generator'
	Write-Host 'output on a specified Microsoft SQL Server instance.'
    Write-Host;
    Write-Host '--------------------------------------------------------------------------------'
    Write-Host;
    ...
    Write-Host 'Specify the server instance where the load control execution files should be'
	Write-Host 'executed.'
    ...
    Write-Host 'Specify the path where the load control execution files to execute are located.'
...
{
	$Scripts = Get-ChildItem -Path "$ScriptPath\*" -Include LoadControl_Execution_*.sql | Foreach-Object { $_.FullName } #compile script list
	if($Scripts.Count -gt 0) #check if any script was found
	{
		Write-Host
		Write-Host "Processing load control execution script in $ScriptPath ..."
